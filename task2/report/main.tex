...
\begin{enumerate}[leftmargin=1.6em, itemsep=0.3em]
    ...
    across $N=128$ to $3072$, with no strong monotonic trend. Prefetching
    helps by a similar relative margin whether the working set fits in
    cache or spills well past LLC.
    \item \textbf{Best prefetch distance:} $64$ floats (mean speedup $1.33\times$
    across $N{=}1024,2048$), narrowly ahead of $32$ ($1.33\times$); distance
    $16$ is an outlier low ($1.12\times$).
    ...
    Differences between hints are small compared to the distance sweep.
    ...
    loads issued); L2 and LLC misses vary more, generally lower at
    distance$=64$/hint$=$T1 than at other settings, consistent with those
    prefetches landing closer to point of use.
    ...
    1.46\times$). Software prefetching substitutes for the missing hardware
    prefetcher; \texttt{naive} has no such substitute. LLC misses jump
