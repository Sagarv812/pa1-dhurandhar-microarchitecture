...
\subsection{Questions}
\begin{enumerate}[leftmargin=1.6em, itemsep=0.3em]
    \item \textbf{Instruction count change (naive $\to$ SIMD):} report the
    observed change and justify it.
    \item \textbf{Speedup:} how much was achieved, what contributed to it (or
    what limited it).
    \item \textbf{Intrinsics used:} which SIMD intrinsics, and the justification
    for each choice.
\end{enumerate}

\begin{todobox}[Answers]
    Answer the three questions above using Tables~\ref{tab:simd-icount},
    \ref{tab:simd-speedup} and Figure~\ref{fig:simd-speedup-vs-size}.
\end{todobox}
