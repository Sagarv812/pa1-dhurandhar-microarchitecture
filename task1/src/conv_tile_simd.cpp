...
void conv_tile_simd(const float* in, float* out, const float* ker,
                    int H, int W, int K) {
    const int p = K / 2;
    ...
    for (int ty = 0; ty < H; ty += T) {
        ...
        for (int tx = 0; tx < W; tx += T) {
            ...
            for (int oy = ty; oy < oy_end; oy++) {
                for (int ox = tx; ox < ox_end; ox+=step) {
...
// void conv_optimized_1(const float* in, float* out, const float* ker,
//                     int H, int W, int K) {
//     const int p = K / 2;
//     const int in_stride = W + 2 * p;
//     const int step = 8;
